...
// typedef visioncpp::utils::cimg Backend;
// #else
// #include "opencv_backend.hpp"
// // using namespace visioncpp::utils::opencv;
// typedef visioncpp::utils::opencv Backend;
// #endif
//
// namespace visioncpp {
// namespace utils {
...
//   IOHandler() : backend() {}
//
//   /* Constructor receives the filename path, which initialize input and
//   output
//    * memories */
//   IOHandler(const char *filename) {
//     backend.initMemory<COLS, ROWS, CHANNELS, T>(
//         filename, output_ptr, outputImage, input_ptr, inputImage);
//   }
//
...
//     int width = COLS;
//     int height = ROWS;
//     int image[width * height];  // first number here is pixels in my image, 3
//     is
//                                 // for RGB values
//
...
//         *(fbp + location + 1) =
//             image[countpixel];  //*(fbp + location + 1) =
//             image[countpixel][1];
//         *(fbp + location + 2) =
//             image[countpixel];  //*(fbp + location + 2) =
//             image[countpixel][0];
//         //*(fbp + location + 3) = 0;
//         countpixel++;
//       }
...
//     int width = COLS;
//     int height = ROWS;
//     int image[width * height][3];  // first number here is pixels in my
//     image, 3
//                                    // is for RGB values
//
//     //	printf("width %d, height %d \n", width, height);
...
//                    (y + vinfo.yoffset) * finfo.line_length;
//         *(fbp + location) =
//             image[countpixel][0];  //*(fbp + location) =
//             image[countpixel][2];
//         *(fbp + location + 1) =
//             image[countpixel]
...
//     int image[230400][3];  // first number here is 236160 pixels in my image,
//     3
//                            // is for RGB values
//     FILE *streamIn;
...
//     int byte;
//     int count = 0;
//     for (int i = 0; i < 54; i++) byte = getc(streamIn);  // strip out BMP
//     header
//
//     for (int i = 0; i < 230400; i++) {  // foreach pixel
//       image[i][2] = getc(streamIn);     // use BMP 24bit with no alpha
//       channel image[i][1] =
