...
struct RGBToGREY_U8 {
  ...
  visioncpp::pixel::U8C1 operator()(visioncpp::pixel::U8C3 in) {
    //  because it is saved cv in our rgb format then b stored in r
    // and r stored in b.
    // luminance , the most accurate one
    // printf("%d\n", in[0]);/
    auto x = visioncpp::OP_U8C3ToF32C3()(in);
    auto myvl = 0.299f * x[0] + 0.587f * x[1] + 0.114f * x[2];

    return visioncpp::OP_FloatToU8C1()(myvl);
  }
};
...
int main(int argc, char **argv) {
  // input data
  cv::Mat input = cv::imread("../media/chessboard.png");
  ...
  cv::resize(input, input, cv::Size(COLS, ROWS), 0, 0, cv::INTER_CUBIC);
  {
    ...
    auto in = visioncpp::terminal<visioncpp::pixel::U8C3, COLS, ROWS,
    ...
    for (size_t i = 0; i < cnt; i++) {
      ...
      auto end = visioncpp::internal::tools::get_current_time();

      if (i != 0) {
        t += visioncpp::internal::tools::get_elapse_time(begin, end);
      }
    }
  }
  ...
  std::cout << t / (cnt - 1) << " seconds" << std::endl;
